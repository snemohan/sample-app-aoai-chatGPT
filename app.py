...
from backend.auth.auth_enable import requires_auth
# import httpx
from quart import (
    Blueprint,
    Quart,
...
from backend.azure_data.azure_file import (
    conversation_internal
)
# from backend.azure_data.constants import CLOUD_PROVIDER

from backend.aws_data.chatbot_api import KendraSearcher, kendra_searcher
# from backend.aws_data.sm_chat import conversation_internal_sm
...
# @requires_auth(roles=["userRole","admin"])
...
@bp.route("/conversation/<string:application_id>/<string:run_id>", methods=["POST"])
# @requires_auth(roles=["userRole","admin","adminRole"])
async def conversation(application_id:str,run_id:str):
    if CLOUD_PROVIDER=="azure":
        if not request.is_json:
            return jsonify({"error": "request must be json"}), 415
        request_json = await request.get_json()
        print("request_json=",request_json)  # request_json= {'messages': [{'role': 'user', 'content': 'Hi'}]}
        return await conversation_internal(request_json,application_id,run_id) 
    elif CLOUD_PROVIDER=="aws":
        try:
            if application_id is None or run_id is None:
            ...
            if not request.is_json:
                return jsonify({"error": "request must be json"},415)
            request_json = await request.get_json()
            # print("request_json aws=",request_json)
            # return await conversation_internal(request_json,application_id,run_id)
        ...
        response = await kendra_searcher.search(request_json, application_id, run_id)
        # print("response AWS = ",response)
        if (response is None):
            logging.debug("Data not found in application")
            return jsonify({"error": "Data not found in application"},404)
        else:
            return response

        # return await conversation_internal_sm(request_json,application_id,run_id) 
# ====================================================================================================
